...
const getItems = async (req, res) => {
  ...
  try {
    const queryText = 'SELECT * FROM items WHERE "userId" = $1 ORDER BY "buyDate" DESC, "createdAt" DESC;';
  ...
  try {
    const queryText = `
      INSERT INTO items (
        ...
        "deliveryName", "deliveryAddr", "deliveryPhone", "trackingNum", "userId"
...
const deleteItem = async (req, res) => {
  ...
  const userId = req.user.id;
  try {
    const result = await db.query('DELETE FROM items WHERE "id" = $1 AND "userId" = $2 RETURNING *;', [id, userId]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Item not found or unauthorized' });
    }
